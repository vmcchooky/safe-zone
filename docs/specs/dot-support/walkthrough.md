...
> **Lưu ý về trạng thái hiện tại:** Đây là báo cáo lịch sử. Repo hiện tại vẫn có mã DoT trong `cmd/dns-resolver/main.go`, nhưng không còn file test riêng `cmd/dns-resolver/main_test.go`; các kết quả test bên dưới không nên được hiểu là test hiện đang tồn tại trong working tree.
...
*   **[MODIFY] Bộ test DoT (file lịch sử không còn trong repo hiện tại):**
