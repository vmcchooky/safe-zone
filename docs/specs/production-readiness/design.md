...
- `scripts/ops/safe-zone.sh` and `scripts/ops/safe-zone.ps1` are the operator backup/restore entrypoints. They snapshot Redis, SQLite, `.env`, and Caddy config, then optionally upload the backup directory with `rclone` when `SAFE_ZONE_RCLONE_REMOTE` is configured.
- `scripts/ops/check-production-ports.sh` and `scripts/ops/public-edge-smoke.sh` are the release-gate scripts for the public edge.
