...
Hệ thống thêm `AssessedAt` vào `analysisCacheEntry` (chỉ worker enrichment và OSINT-apply stamp; final SET của `analyze` CỐ TÌNH không stamp), `QueuedAt` vào `enrichmentJob` (đặt lúc enqueue, sau cache write và sau OSINT), helper `entryAssessedAt`/`jobQueuedAt`, và guard trong `processEnrichmentJob` bỏ write khi entry mới hơn snapshot. `currentFeedRevision`/`currentBrandRevision` trả thêm cờ known (phân biệt `redis.Nil` với lỗi); `entryMatchesRevision` gom logic hit-check cũ thành hàm thuần có test. OSINT thêm `sourceRevision` (sha256 sources+trusted) và `evidenceCacheKey`. Nếu dùng AI agent: mô hình `muse-spark`, chiến lược trích helper thuần trước để test ma trận revision, kiểm soát bằng replay ordering cũ đảo expectation, vai trò con người duyệt.

Bài học CI (panic `close of closed channel` ở `TestSuspiciousDomain...` trên runner GitHub, trong khi local xanh): hai lỗi chồng nhau. (1) `AssessedAt` trên final SET khiến worker skip vĩnh viễn + vòng re-enqueue (đã bỏ stamp — bản vá đầu tiên). (2) Sâu hơn và tiền tồn tại: worker đọc entry TRƯỚC khi final SET của cùng request hạ cánh (race worker-goroutine vs request-goroutine), thấy entry trống rồi ghi đè/quay vòng lookup. Sửa gốc bằng thứ tự: enqueue SAU cache write và SAU OSINT (snapshot worker bao gồm mọi signal đồng bộ), cộng snapshot post-OSINT ở hit path — mỗi episode đúng một lookup một cách deterministic. Test lặp count=10 và `-race` xác nhận; giữ nguyên strictness của test (không bọc `sync.Once`) để hồi quy ordering sẽ lại đỏ ngay.
