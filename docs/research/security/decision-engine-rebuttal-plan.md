...
| 2026-09-22 | Offensive review → top-3 fix đã merge (branch `codex/security-top3-fixes`, PR riêng): (F1) Replace sync `Valid==0` fail giữ feed cũ thay vì `Delete` key live (`sync.go:312`) + regression test đỏ-trước-xanh-sau; (F2) từ chối `http://` feed mặc định, opt-out tường minh qua `AllowInsecureHTTP`/`-allow-insecure-http`/`SAFE_ZONE_FEED_ALLOW_INSECURE_HTTP` (CLI + feed-syncd + agent) + 2 tests; (F3) `/v1/analyze/raw` yêu cầu auth (đóng backlog PROMPT-NEXT) + router 401 test. Full `internal/...` + `-race` + eval 4/4 xanh, 0 drift decision engine. Không deploy trong PR (chờ gate) | Muse Spark |
| 2026-09-23 | Review độc lập round 2 → follow-up đã code (branch `codex/security-review-followup`): F2-BYPASS xác minh đúng (redirect `https→http` lọt gate chuỗi ban đầu) → chặn downgrade trong `OpenSourceResponseWithin` (opt-out theo `allowInsecureHTTP`) + e2e TLS test (302 xuống http bị chặn, https→https thông); force_osint anon xác minh đúng (không phải PANIC — là forced outbound OSINT, Medium-Low) → middleware `AttachAuthIdentityFunc` + gate `forceOSINTForRequest`, dashboard operator (session) không ảnh hưởng; Low feedback → `RequireAuthFunc` + runbook curl Bearer; ruling giữ nguyên block-report anon (by design) và guest raw (authenticated). Full `internal/...` + `-race` + eval 4/4 xanh | Muse Spark |
| 2026-09-23 | Siết test round-2 (branch `codex/security-test-tightening`): rule downgrade refactor vào `netguard.RedirectPolicyHTTPS` (tái dùng, unit test) thay closure inline; F1 test + assert staging sạch; reject test thành non-DryRun chứng minh gate-trước-dial; feed-syncd parse/passthrough test; handlers analyze-public-preserved + middleware/gate unit tests; router method/path matrix. Full `internal/...` + `cmd/...` + `-race` + eval 4/4 xanh | Muse Spark |
