...
export function GroupModal({ isOpen, onClose, onSave, group }: GroupModalProps) {
  ...
  const [strictPhishing, setStrictPhishing] = useState(false);
  const [strictMalware, setStrictMalware] = useState(false);
  ...
  useEffect(() => {
    if (isOpen) {
      if (group) {
        ...
        setBlockCategories(group.block_categories || []);
        setStrictPhishing(group.strict_phishing);
        setStrictMalware(group.strict_malware);
      } else {
        setName('');
        setDescription('');
        setBlockCategories([]);
        setStrictPhishing(false);
        setStrictMalware(false);
      }
      setError(null);
    }
  }, [isOpen, group]);
              ...
              <form id="group-form" onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all"
                    />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {CATEGORIES.map(cat => {
                      const active = blockCategories.includes(cat.id);
                      const Icon = cat.icon;
                      return (
                        <button
                          type="button"
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <button
                      type="button"
                      onClick={() => setStrictPhishing(!strictPhishing)}
                      className={`flex items-center justify-between w-full p-4 rounded-2xl border transition-all ${
                        strictPhishing ? 'bg-emerald-50/50 border-emerald-200' : 'bg-white border-slate-200 hover:border-slate-300'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-xl ${strictPhishing ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
                          <Shield size={18} />
                        </div>
                        <div className="text-left">
                          <div className={`font-semibold text-sm ${strictPhishing ? 'text-emerald-900' : 'text-slate-700'}`}>
                            Strict Phishing
                          </div>
                          <div className="text-xs text-slate-500 mt-0.5">Escalate suspicious login and impersonation signals.</div>
                        </div>
                      </div>
                      <div className={`w-11 h-6 rounded-full transition-colors flex items-center px-1 ${strictPhishing ? 'bg-emerald-500' : 'bg-slate-200'}`}>
                        <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${strictPhishing ? 'translate-x-5' : 'translate-x-0'}`} />
                      </div>
                    </button>

                    <button
                      type="button"
                      onClick={() => setStrictMalware(!strictMalware)}
                      className={`flex items-center justify-between w-full p-4 rounded-2xl border transition-all ${
                        strictMalware ? 'bg-rose-50/50 border-rose-200' : 'bg-white border-slate-200 hover:border-slate-300'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-xl ${strictMalware ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-500'}`}>
                          <ShieldAlert size={18} />
                        </div>
                        <div className="text-left">
                          <div className={`font-semibold text-sm ${strictMalware ? 'text-rose-900' : 'text-slate-700'}`}>
                            Strict Malware
                          </div>
                          <div className="text-xs text-slate-500 mt-0.5">Apply stronger blocking for payload and abuse indicators.</div>
                        </div>
                      </div>
                      <div className={`w-11 h-6 rounded-full transition-colors flex items-center px-1 ${strictMalware ? 'bg-rose-500' : 'bg-slate-200'}`}>
                        <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${strictMalware ? 'translate-x-5' : 'translate-x-0'}`} />
                      </div>
                    </button>
                  </div>
                </div>
