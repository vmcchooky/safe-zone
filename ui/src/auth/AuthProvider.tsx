...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (username: string, password: string) => {
    ...
    try {
      // Preload the Analysis chunk alongside the auth request and the single
      // intentional minimum loader duration — one wait, one loader chain.
      const analysisChunk = preloadAnalysisRoute();
      const req = apiJSON<{ status: string }>('/v1/auth/login', { username, password }, { method: 'POST' });
      await Promise.all([req, minLoaderDelay(MIN_LOADER_DURATION_MS), analysisChunk]);
